...
## D-027 · The specimen is CC0 gravel, fetched through scikit-image

§2 wants "a CC0 macro photograph (botanical or mineral)" refracted through Plate IV's cloth,
and §0.4 forbids unlicensed imagery. This sandbox refuses CONNECT to every image host probed —
`upload.wikimedia.org`, `commons.wikimedia.org`, `images.pexels.com`, `cdn.jsdelivr.net` — and
`api.github.com` answers but is scoped to this repository, so there is no search either. The
only route out is `raw.githubusercontent.com/<owner>/<repo>/<ref>/<path>`, which needs the path
known in advance.

So the image had to come from a repository that both hosts the file and documents its
provenance in the same tree. `scikit-image` does: `skimage/data/_fetchers.py` names the source
of every bundled image, its licence, and the transformation applied. `gravel.png` is
ambientCG's "Gravel 04" under CC0, rescaled to 1024² and cropped to the top-left 512², greyscale
uint8. The full chain is in `CREDITS.md`, because a licence claim with no traceable origin is
worth nothing.

Considered and rejected: `KhronosGroup/glTF-Sample-Assets` (reachable, per-model licences, but
3-D models rather than photography) and generating a synthetic specimen (would satisfy nothing —
§2 is explicit that this is *the* distorted-image requirement and the one place a photograph
appears in the site, so a procedural substitute fails the requirement rather than meeting it
differently).

The image is greyscale, which is not a compromise: see D-028.

---

## D-028 · The cloth refracts spectrally, so its colour obeys the same rule as Plate II

§2 Plate II sets the rule the whole piece lives under — all colour in WEFT is produced by
refraction, and there is no brand accent. The specimen is a greyscale photograph. Displacing a
greyscale sample by the surface normal would make Plate IV the only monochrome plate in the
sequence, and tinting it would break the rule outright.

So the displacement is wavelength-dependent, through the same Cauchy relation Plate II uses and
from the same endpoints (n = 1.78 at 380 nm, 1.46 at 740 nm, D-014). Each wavelength lands on a
slightly different part of the specimen; where the surface is steep those samples disagree, and
the disagreement *is* the colour. Flat areas stay grey because every wavelength lands on the
same texel.

That is how a real refracting surface behaves, and it means the colour in this plate is
generated by the same physics as the colour in Plate II rather than by a second mechanism that
happens to resemble it. It also makes the greyscale source an advantage: a colour photograph
would have its own hues competing with the ones the geometry produces, and it would be
impossible to tell which was which.

Sampling is mirrored rather than clamped or wrapped. Clamping smears the specimen's edge pixels
into streaks wherever a fold runs off it; wrapping puts a hard seam through the middle of one.

---

## D-029 · The cloth steps at a fixed 1/60 s, not at the frame delta

Every other simulation in the piece takes the frame delta — clamped, but real. The cloth does
not, and the difference is worth stating because it looks like an oversight.

A Verlet integrator has no velocity term: motion is the difference between the last two
positions. Damping is therefore applied per *step*, not per second, and so is the convergence of
the Jacobi relaxation that follows it. Feed that loop a variable dt and the cloth's stiffness
changes with the frame rate — a slow frame produces a visibly slacker sheet and a fast one a
starchier one. Clamping does not fix it; only a constant step does.

The trade, stated plainly: on a device that cannot hold 60 fps the plate runs in slow motion
rather than behaving like a different material. That is the right way round. A material whose
properties depend on the visitor's hardware is not a material.
